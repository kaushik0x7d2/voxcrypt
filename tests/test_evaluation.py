...
class TestFARFRR:
    ...
    def test_needs_both_classes(self):
        with pytest.raises(ValueError):
            compute_far_frr(np.array([0.5, 0.5]), np.array([1, 1]))
...
class TestEER:
    ...
    def test_returns_valid_threshold(self):
        scores = np.concatenate(
            [np.random.normal(0.3, 0.1, 50), np.random.normal(0.7, 0.1, 50)]
        )
        labels = np.array([0] * 50 + [1] * 50)
...
class TestRecommendThreshold:
    def test_low_far_target(self):
        scores = np.concatenate(
            [np.random.normal(0.2, 0.1, 100), np.random.normal(0.8, 0.1, 100)]
        )
        labels = np.array([0] * 100 + [1] * 100)
...
class TestEvaluationReport:
    def test_report_keys(self):
        np.random.seed(42)
        scores = np.concatenate(
            [np.random.normal(0.3, 0.15, 50), np.random.normal(0.7, 0.15, 50)]
        )
        labels = np.array([0] * 50 + [1] * 50)
