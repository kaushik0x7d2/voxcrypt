...
def main():
    ...
    model.load_state_dict(torch.load(
        os.path.join(demo_dir, "encrypted_model.pt"), weights_only=True))
    model.eval()
    ...
    print(f"  Both voiceprints encrypted together — server never sees")
    print(f"  individual embeddings. Comparison happens under FHE.")
    print(f"  Model: EncryptedVerifyNet (80->128->64->2, GELU)")
    print()
    ...
    print(f"\n{'='*65}")
    print(f"  RESULTS — Encrypted Template Protection")
    print(f"{'='*65}")
    ...
    print(f"\n  Novel contribution:")
    print(f"    Both voiceprints encrypted together as [emb_A || emb_B]")
    print(f"    Comparison learned by neural network under FHE")
    print(f"    Server never sees individual biometric templates")
    print(f"    Improves over Nautsch et al. (2018) Paillier partial HE")
