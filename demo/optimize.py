...
from speaker_verify.dataset import download_librispeech, scan_speakers, generate_pairs, build_dataset
...
EXPERIMENTS = [
    ...
    # FHE-aware noise
    {"name": "fhe_noise_01", "n_pairs": 5000, "epochs": 200, "lr": 1e-3, "noise_std": 0.1},
    {"name": "fhe_noise_03", "n_pairs": 5000, "epochs": 200, "lr": 1e-3, "noise_std": 0.3},
    {"name": "fhe_noise_05", "n_pairs": 5000, "epochs": 200, "lr": 1e-3, "noise_std": 0.5},
    # learning rates
    {"name": "lr_5e4", "n_pairs": 5000, "epochs": 300, "lr": 5e-4, "noise_std": 0.1},
    {"name": "lr_5e3", "n_pairs": 5000, "epochs": 200, "lr": 5e-3, "noise_std": 0.1},
    # longer training
    {"name": "long_train", "n_pairs": 5000, "epochs": 500, "lr": 5e-4, "noise_std": 0.1},
]
...
def main():
    ...
    for i, config in enumerate(experiments):
        ...
        print(f"    n_pairs={config['n_pairs']}, epochs={config['epochs']}, "
    ...
    with open(results_path, "w", newline="") as f:
        ...
        writer.writerow(["experiment", "n_pairs", "epochs", "lr", "noise_std",
                         "val_acc", "train_acc", "best_epoch", "time_s"])
        for r in results:
            writer.writerow([r["name"], r["n_pairs"], r["epochs"], r["lr"],
                             r["noise_std"], f"{r['val_acc']:.4f}",
                             f"{r['train_acc']:.4f}", r["best_epoch"],
                             f"{r['time']:.1f}"])
