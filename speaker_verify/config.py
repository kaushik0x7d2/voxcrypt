...
class Config:
    ...
    def from_env(cls):
        ...
        config.server.request_timeout = int(os.environ.get(
            "ORION_VOICE_REQUEST_TIMEOUT", "120"))
        config.server.max_payload_mb = int(os.environ.get(
            "ORION_VOICE_MAX_PAYLOAD_MB", "50"))
        config.server.cors_origins = os.environ.get(
            "ORION_VOICE_CORS_ORIGINS", "")
        config.server.production = os.environ.get(
            "ORION_VOICE_PRODUCTION", "").lower() in ("true", "1", "yes")

        # Security
        config.security.api_key = os.environ.get("ORION_VOICE_API_KEY", "")
        config.security.require_auth = os.environ.get(
            "ORION_VOICE_REQUIRE_AUTH", "").lower() in ("true", "1", "yes")
        config.security.tls_cert = os.environ.get("ORION_VOICE_TLS_CERT", "")
        config.security.tls_key = os.environ.get("ORION_VOICE_TLS_KEY", "")
        config.security.rate_limit = int(os.environ.get(
            "ORION_VOICE_RATE_LIMIT", "10"))
        config.security.rate_limit_window = int(os.environ.get(
            "ORION_VOICE_RATE_LIMIT_WINDOW", "60"))
        config.security.max_upload_mb = int(os.environ.get(
            "ORION_VOICE_MAX_UPLOAD_MB", "50"))
        config.security.key_password = os.environ.get(
            "ORION_VOICE_KEY_PASSWORD", "")
        config.security.require_hmac = os.environ.get(
            "ORION_VOICE_REQUIRE_HMAC", "").lower() in ("true", "1", "yes")

        # FHE
        config.fhe.config_path = os.environ.get(
            "ORION_VOICE_FHE_CONFIG_PATH", "")
        config.fhe.decision_threshold = float(os.environ.get(
    ...
    def validate(self):
        ...
        if self.security.tls_cert and not os.path.exists(
                self.security.tls_cert):
            errors.append(f"TLS cert not found: {self.security.tls_cert}")

        if self.security.tls_key and not os.path.exists(
                self.security.tls_key):
